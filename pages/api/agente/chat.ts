...
async function getDataSource(): Promise<DataSource> {
  ...
  if (!dbUrl) {
    throw new Error('Variavel de ambiente SUPABASE_DB_URL nao configurada. Adicione no Doppler.')
  }

  if (!dbUrl.startsWith('postgres://') && !dbUrl.startsWith('postgresql://')) {
    throw new Error('SUPABASE_DB_URL com formato invalido. Deve comecar com postgresql:// ou postgres://')
  }
  ...
  try {
    ...
    if (msg.includes('Invalid URL') || msg.includes('ERR_INVALID_URL')) {
      throw new Error('SUPABASE_DB_URL com formato invalido. Verifique a connection string no Supabase Dashboard > Settings > Database.')
    }
    if (msg.includes('ECONNREFUSED') || msg.includes('ETIMEDOUT')) {
      throw new Error('Nao foi possivel conectar ao banco de dados. Verifique se a SUPABASE_DB_URL esta correta e se o banco esta acessivel.')
    }
    if (msg.includes('password authentication failed')) {
      throw new Error('Senha do banco de dados incorreta. Verifique a SUPABASE_DB_URL no Doppler.')
    }
...
function mapErrorMessage(rawMsg: string): string {
  ...
  if (rawMsg.includes('AGENT_ENCRYPTION_KEY')) return 'Chave de encriptacao nao configurada. Contate o administrador.'
  if (rawMsg.includes('Incorrect API key') || rawMsg.includes('invalid x-api-key') || rawMsg.includes('invalid_api_key')) {
    return 'API key invalida ou revogada. Gere uma nova chave e atualize na aba Configuracao.'
  }
  if (rawMsg.includes('insufficient_quota') || rawMsg.includes('rate_limit') || rawMsg.includes('429')) {
    return 'Limite de uso da API excedido. Verifique seu plano ou aguarde alguns minutos.'
  }
  if (rawMsg.includes('model_not_found') || rawMsg.includes('does not exist') || rawMsg.includes('404')) {
    return 'Modelo selecionado nao disponivel. Troque o modelo na aba Configuracao.'
  }
  if (rawMsg.includes('ECONNREFUSED') || rawMsg.includes('ETIMEDOUT') || rawMsg.includes('fetch failed')) {
  ...
  if (rawMsg.includes('401')) return 'Erro de autenticacao com a API. Verifique sua API key na aba Configuracao.'
  if (rawMsg.includes('GRAPH_RECURSION_LIMIT') || rawMsg.includes('Recursion limit')) {
...
const handler = async (req: AuthenticatedRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (configError || !agentConfig) {
      return res.status(400).json({
        success: false,
        message: 'Configure sua API key na aba Configuracao antes de usar o chat.',
      })
    }
    ...
    if (!agentConfig.api_key_encrypted) {
      return res.status(400).json({
        success: false,
        message: 'API key nao configurada. Va na aba Configuracao para adicionar.',
      })
    }
    ...
    const keyPreview = apiKey.length > 8
      ...
      : '***'
    console.log(`[API Agente Chat] Provider: ${agentConfig.provider}, Model: ${agentConfig.model}, Key preview: ${keyPreview}`)
    ...
    let fullResponse = ''
    const sqlQueries: { sql: string; explanation: string; rows_returned: number; execution_time_ms: number }[] = []
    const toolCalls: { tool_name: string; args: Record<string, unknown>; result: unknown }[] = []
    ...
    for await (const event of stream) {
      ...
      if (event.agent) {
        ...
        for (const msg of agentMessages) {
          ...
          if (usageMeta) {
            totalInputTokens += usageMeta.input_tokens || usageMeta.prompt_tokens || 0
            totalOutputTokens += usageMeta.output_tokens || usageMeta.completion_tokens || 0
          }
      ...
      if (event.tools) {
        ...
        for (const msg of toolMessages) {
          ...
          for (let i = toolCalls.length - 1; i >= 0; i--) {
            if (toolCalls[i].tool_name === toolName && toolCalls[i].result === null) {
    ...
    await supabase
      ...
      .update({
        ...
        total_input_tokens: (conversation as Record<string, unknown>).total_input_tokens
          ? Number((conversation as Record<string, unknown>).total_input_tokens) + totalInputTokens
          : totalInputTokens,
        total_output_tokens: (conversation as Record<string, unknown>).total_output_tokens
          ? Number((conversation as Record<string, unknown>).total_output_tokens) + totalOutputTokens
          : totalOutputTokens,
      })
      .eq('id', conversationId)
  } catch (error) {
