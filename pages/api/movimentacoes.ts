...
// Função helper para atualizar estoque via stock-manager (com locking + audit trail)
async function atualizarEstoque(
  supabase: SupabaseClient,
  itens: Array<
    { produto_id: number; quantidade: number; preco_unitario?: number }
  >,
  tipo: "entrada" | "saida",
  ...
  for (const item of itens) {
    ...
    if (!estoque_id) {
      ...
      estoque_id = produto?.estoque_id || 1;
    }

    // Calcular delta (positivo para entrada, negativo para saída)
    const delta = tipo === "entrada" ? item.quantidade : -item.quantidade;
    const tipoOperacao: TipoOperacao = tipo === "entrada" ? "COMPRA" : "AJUSTE";

    // Verificar se já existe registro em produtos_estoques (necessário para adjustProductStock)
    ...
    if (!existing) {
      // Criar registro inicial zerado para que adjustProductStock funcione
      await supabase
        .from("produtos_estoques")
        .insert({
    ...
    // Usar adjustProductStock: com row-level locking + estoques_historico automático
    ...
    if (!result.success) {
      throw new Error(
        `Erro ao ajustar estoque do produto ${item.produto_id}: ${result.error}`,
      );
    }

    // CALCULAR PREÇO MÉDIO PONDERADO para entradas
    if (
      tipo === "entrada" && item.preco_unitario !== undefined &&
      ...
      if (produto) {
        // Estoque total ANTES da entrada = total agora - o que acabamos de adicionar
...
const handler = async (req: AuthenticatedRequest, res: NextApiResponse) => {
  ...
  try {
    if (method === "GET") {
      const { id, page = "1", limit = "10", tipo = "", status = "" } =
        req.query;
      ...
      if (id) {
        const { data, error } = await supabase
          ...
          .select(
            "*, fornecedor:fornecedores(nome), itens:movimentacoes_itens(*, produto:produtos(nome, id))",
          )
          .eq("id", id)
          .single();
      ...
      let query = supabase
        .from("movimentacoes_estoque")
        .select(
          "*, fornecedor:fornecedores(nome), itens:movimentacoes_itens(*, produto:produtos(nome))",
          { count: "exact" },
        );
    ...
    if (method === "POST") {
      ...
      if (itens && Array.isArray(itens) && itens.length > 0) {
        ...
        const { error: itensError } = await supabase.from("movimentacoes_itens")
          .insert(itensInsert);

        if (itensError) {
          // Se falhou ao inserir itens, deletar a movimentação criada
          await supabase.from("movimentacoes_estoque").delete().eq(
            "id",
            movimentacao.id,
          );
          throw new Error(
            `Erro ao inserir itens da movimentação: ${itensError.message}`,
          );
        }
        ...
        try {
          ...
          await supabase.from("movimentacoes_itens").delete().eq(
            "movimentacao_id",
            movimentacao.id,
          );
          await supabase.from("movimentacoes_estoque").delete().eq(
    ...
    if (method === "PUT") {
      ...
      if (
        itens && itens.length > 0 && movimentacaoAtual.itens &&
        movimentacaoAtual.itens.length > 0
      ) {
        try {
          // Reverter estoque da movimentação antiga
          const tipoReversao = movimentacaoAtual.tipo === "entrada"
            ? "saida"
            : "entrada";
          await atualizarEstoque(
            supabase,
            movimentacaoAtual.itens,
            tipoReversao,
            movimentacaoAtual.estoque_id,
            movimentacaoAtual.id,
          );
        } catch (estoqueError) {
          throw new Error(
            `Erro ao reverter estoque antigo: ${
              estoqueError instanceof Error
                ? estoqueError.message
                : "Erro desconhecido"
            }`,
      ...
      const { data: movimentacaoAtualizada, error: updateError } =
        await supabase
          .from("movimentacoes_estoque")
          .update({
            ...
            fornecedor_id: fornecedor_id !== undefined
              ? fornecedor_id
              : movimentacaoAtual.fornecedor_id,
            numero_pedido: numero_pedido !== undefined
              ? numero_pedido
              : movimentacaoAtual.numero_pedido,
            data_movimentacao: data_movimentacao ||
              movimentacaoAtual.data_movimentacao,
            valor_total: valor_total !== undefined
              ? valor_total
              : movimentacaoAtual.valor_total,
            condicao_pagamento: condicao_pagamento ||
              movimentacaoAtual.condicao_pagamento,
            status: status || movimentacaoAtual.status,
            observacoes: observacoes !== undefined
              ? observacoes
              : movimentacaoAtual.observacoes,
      ...
      if (itens && itens.length > 0) {
        ...
        try {
          await atualizarEstoque(
            ...
            movimentacaoAtualizada.estoque_id,
            movimentacaoAtualizada.id,
          );
        } catch (estoqueError) {
          throw new Error(
            `Erro ao aplicar novo estoque: ${
              estoqueError instanceof Error
                ? estoqueError.message
                : "Erro desconhecido"
            }`,
    ...
    if (method === "DELETE") {
      ...
      if (movimentacao.itens && movimentacao.itens.length > 0) {
        try {
          ...
          const tipoReversao = movimentacao.tipo === "entrada"
            ? "saida"
            : "entrada";
          await atualizarEstoque(
            supabase,
