...
import { withSupabaseAuth, type AuthenticatedRequest } from '@/lib/supabase-middleware'
...
const handler = async (req: AuthenticatedRequest, res: NextApiResponse) => {
  ...
  try {
    if (method === 'GET') {
      ...
      if (!accessProfile) {
        return res.status(403).json({ success: false, message: 'Usuário sem perfil configurado' })
      }
      ...
      const hasStart = typeof data_inicio === 'string' && data_inicio.trim().length > 0
      const hasEnd = typeof data_fim === 'string' && data_fim.trim().length > 0
      const hasCliente = typeof cliente_id === 'string' && cliente_id.trim().length > 0

      const requestedVendorId = typeof vendedor_id === 'string' && vendedor_id.trim().length > 0
      ...
      if (!accessProfile.canViewAllSales) {
        if (accessProfile.vendedorId) {
          ...
          return res.status(200).json(buildEmptyPagination(pageNum, limitNum))
        }
      }
    ...
    if (method === 'POST') {
      ...
      const itensInsert = vendaProcessada.itens.map((item) => ({
        ...
        aliquota_icms: (item.icms_st_aliquota || item.icms_aliquota) / 100,
      }))
      ...
      if (parcelas && Array.isArray(parcelas) && parcelas.length > 0) {
        ...
        if (!parcelasError && parcelasCreated) {
          ...
          await supabase.from('transacoes').insert(transacoesToInsert)
        }
      } else {
        const { data: categoriaVendas } = await supabase
          .from('categorias_financeiras')
