...
const FULL_COLUMN_SET = 'id, email, role, tipo_usuario, roles, permissoes, permissoes_custom, vendedor_id'
...
const buildUserQuery = (
  ...
  if (query.supabaseUserId) {
    return builder.eq('supabase_user_id', query.supabaseUserId).maybeSingle()
  }
...
export const fetchUserAccessProfile = async (
  ...
  let { data, error } = await buildUserQuery(supabase, query, FULL_COLUMN_SET)

  if (isMissingColumnError(error)) {
    usedLegacyQuery = true
    ;({ data, error } = await buildUserQuery(supabase, query, LEGACY_COLUMN_SET))
  }
  ...
  const canViewAllSales = usedLegacyQuery
    ? true
    : VIEW_ALL_ROLES.has(tipoUsuario) || permissions.vendas_visualizar_todas === true
