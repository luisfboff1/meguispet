...
export default function VendasChart({ data, loading = false }: VendasChartProps) {
  ...
  const maxVendas = Math.max(...data.map(d => d.vendas || 0), 0)
  const maxReceita = Math.max(...data.map(d => d.receita || 0), 0)
    ...
    <div className="flex h-64 w-full flex-col">
      <div className="relative flex-1 overflow-visible">
        <div className="absolute inset-0 flex items-end gap-3">
          {data.map((item, index) => {
            const vendasCount = item.vendas || 0
            const receitaValue = item.receita || 0
            const vendasHeight = Math.round((vendasCount / safeMaxVendas) * 160)
            const receitaHeight = Math.round((receitaValue / safeMaxReceita) * 120)
              ...
              <div key={index} className="flex w-full flex-1 flex-col items-center gap-2">
                <div className="flex w-full items-end justify-center gap-1">
                  <div
                    ...
                    style={{ height: Math.max(vendasHeight, vendasCount > 0 ? 4 : 0) }}
                    aria-label={`Vendas em ${item.data}: ${vendasCount}`}
                  >
                    {vendasCount > 0 && (
                  ...
                  <div
                    ...
                    style={{ height: Math.max(receitaHeight, receitaValue > 0 ? 4 : 0) }}
                    aria-label={`Receita em ${item.data}: R$ ${receitaValue.toFixed(2)}`}
                  >
                    {receitaValue > 0 && (
                      <span className="absolute -top-6 rounded-full bg-white/90 px-1.5 py-0.5 text-[10px] font-medium shadow-sm">
                        R$ {receitaValue.toFixed(0)}
                      </span>
                    )}
