...
if (process.env.NODE_ENV === "development") {
  api.interceptors.request.use((config) => {
    const payloadPreview = typeof config.data === "string"
      ? config.data
...
export const dashboardService = {
  ...
  async getVendasPeriodo(
    days: number = 7,
  ): Promise<ApiResponse<DashboardVendasDia[]>> {
...
export const clientesService = {
  async getAll(
    ...
    const response = await api.get(
      `/clientes?page=${page}&limit=${limit}&includeInactive=${includeInactive}`,
    );
    return response.data;
  },
  ...
  async update(
    id: number,
    cliente: Partial<Cliente> | ClienteFormPayload,
  ...
  async search(term: string, limit = 20): Promise<PaginatedResponse<Cliente>> {
    const response = await api.get(
      `/clientes?search=${encodeURIComponent(term)}&limit=${limit}`,
    );
    return response.data;
  },
};
...
export const produtosService = {
  ...
  async update(
    id: number,
    produto: Partial<Produto> | ProdutoFormPayload,
  ...
  async updateStock(id: number, quantidade: number): Promise<ApiResponse> {
    const response = await api.patch(`/produtos?id=${id}`, {
      estoque: quantidade,
    });
    return response.data;
  },
...
export const historicoPrecosService = {
  async getByProdutoId(produtoId: number, limit = 20): Promise<ApiResponse> {
    ...
    const response = await api.get(
      `/historico-precos?produto_id=${produtoId}&limit=${limit}`,
    );
    return response.data;
  },
};
...
export const vendasService = {
  ...
  async getByVendedorId(
    vendedorId: number,
    page = 1,
    limit = 100,
  ): Promise<ApiResponse<Venda[]>> {
    const response = await api.get(
      `/vendas?vendedor_id=${vendedorId}&page=${page}&limit=${limit}`,
    );
    return response.data;
  },
  ...
  async update(
    id: number,
    venda: Partial<Venda> | VendaFormPayload,
  ): Promise<ApiResponse<Venda>> {
...
export const estoquesService = {
  ...
  async create(
    payload: { nome: string; descricao?: string; ativo?: number },
  ): Promise<ApiResponse> {
...
export const formasPagamentoService = {
  async getAll(
    activeOnly = false,
  ): Promise<ApiResponse<FormaPagamentoRegistro[]>> {
    const response = await api.get(
      `/formas_pagamento${activeOnly ? "?active=1" : ""}`,
    );
    return response.data;
  },

  async create(
    payload: { nome: string; ativo?: number; ordem?: number },
  ): Promise<ApiResponse> {
...
export const condicoesPagamentoService = {
  async getAll(activeOnly = false): Promise<ApiResponse<CondicaoPagamento[]>> {
    const response = await api.get(
      `/condicoes_pagamento${activeOnly ? "?active=1" : ""}`,
    );
    return response.data;
  },
  ...
  async update(
    id: number,
    payload: Partial<CondicaoPagamentoForm>,
  ): Promise<ApiResponse<CondicaoPagamento>> {
...
export const vendedoresService = {
  ...
  async create(
    vendedor: Omit<Vendedor, "id" | "created_at" | "updated_at">,
  ): Promise<ApiResponse<Vendedor>> {
...
export const authService = {
  ...
  async signup(
    ...
    password: string,
    nome: string,
    tipo_usuario: import("@/types").UserRole = "operador",
  ): Promise<ApiResponse<{ user: Usuario; auth_user_id: string }>> {
...
export const fornecedoresService = {
  ...
  ): Promise<ApiResponse<Fornecedor>> {
...
export const movimentacoesService = {
  ...
  async create(
    movimentacao: MovimentacaoFormPayload,
    ...
    const payload = {
      ...
      valor_total: movimentacao.produtos.reduce(
        (acc, p) => acc + (p.valor_total ?? (p.quantidade * p.preco_unitario)),
        0,
      ),
    };
  ...
  async updateStatus(id: number, status: string): Promise<ApiResponse> {
    const response = await api.put(`/movimentacoes/${id}/status`, {
      id,
      status,
    });
    return response.data;
...
export const ajusteEstoqueService = {
  async ajustarDireto(payload: {
    ...
    const response = await api.post("/estoque-ajuste-direto", payload);
    return response.data;
  },
};
...
export const estoqueHistoricoService = {
  async getByProdutoId(
    ...
    let url = `/estoque-historico?produto_id=${produtoId}&limit=${limit}`;
...
export const transacoesService = {
  async getAll(
    ...
    dataInicio?: string,
    dataFim?: string,
  ): Promise<PaginatedResponse<TransacaoFinanceira>> {
  ...
  async update(
    id: number,
    transacao: any,
  ): Promise<ApiResponse<TransacaoFinanceira>> {
...
export const categoriasFinanceirasService = {
  ...
  async create(
    categoria: CategoriaFinanceiraForm,
  ): Promise<ApiResponse<CategoriaFinanceira>> {
  ...
  async update(
    id: number,
    categoria: Partial<CategoriaFinanceiraForm>,
  ): Promise<ApiResponse<CategoriaFinanceira>> {
    const response = await api.put(`/categorias-financeiras/${id}`, {
      ...categoria,
      id,
    });
    return response.data;
...
export const transacoesRecorrentesService = {
  ...
  async create(
    transacao: TransacaoRecorrenteForm,
  ): Promise<ApiResponse<TransacaoRecorrente>> {
  ...
  async update(
    id: number,
    transacao: Partial<TransacaoRecorrenteForm>,
  ): Promise<ApiResponse<TransacaoRecorrente>> {
    const response = await api.put(`/transacoes-recorrentes/${id}`, {
      ...transacao,
      id,
    });
    return response.data;
...
export const vendaParcelasService = {
  ...
  async create(
    parcelas: VendaParcelaInput[],
    vendaId: number,
  ): Promise<ApiResponse<VendaParcela[]>> {
  ...
  async update(
    id: number,
    parcela: Partial<VendaParcela>,
  ): Promise<ApiResponse<VendaParcela>> {
    const response = await api.put(`/venda-parcelas/${id}`, { ...parcela, id });
    return response.data;
  },

  async updateDataVencimento(
    id: number,
    data_vencimento: string,
  ): Promise<ApiResponse<VendaParcela>> {
    const response = await api.patch(`/venda-parcelas/${id}/data-vencimento`, {
      data_vencimento,
    });
    return response.data;
  },
  ...
  async marcarComoPaga(
    ...
    const response = await api.patch(`/venda-parcelas/${id}/pagar`, {
      data_pagamento,
    });
    return response.data;
  },
