...
REM =============================================
@REM cd database; ./backup-complete.bat
...
set TIMESTAMP=%datetime:~0,4%_%datetime:~4,2%_%datetime:~6,2%_%datetime:~8,6%

REM Definir diretório de backup final
...
echo ========================================
echo 🔄 BACKUP COMPLETO DO BANCO DE DADOS - MEGUISPET
echo ========================================
...
echo.
echo ╔════════════════════════════════════════════╗
echo ║   SCHEMA: public (dados do MeguisPet ERP) ║
echo ╚════════════════════════════════════════════╝
echo.

echo 📦 [1/6] Backup COMPLETO do schema public...
pg_dump -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -n public -F p -b -v -f "%BACKUP_DIR%\meguispet_full_%TIMESTAMP%.sql"
if %errorlevel% equ 0 (echo ✅ OK) else (echo ❌ ERRO & pause & exit /b 1)

echo 🏗️  [2/6] Backup ESTRUTURA do schema public...
pg_dump -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -n public -F p -s -v -f "%BACKUP_DIR%\meguispet_structure_%TIMESTAMP%.sql"
if %errorlevel% equ 0 (echo ✅ OK) else (echo ❌ ERRO & pause & exit /b 1)

echo 📊 [3/6] Backup DADOS do schema public...
pg_dump -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -n public -F p -a -v -f "%BACKUP_DIR%\meguispet_data_%TIMESTAMP%.sql"
if %errorlevel% equ 0 (echo ✅ OK) else (echo ❌ ERRO & pause & exit /b 1)
...
echo    ✅ meguispet_full_%TIMESTAMP%.sql
echo    ✅ meguispet_structure_%TIMESTAMP%.sql
echo    ✅ meguispet_data_%TIMESTAMP%.sql
echo.
